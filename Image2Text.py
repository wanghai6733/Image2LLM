...
from llm_search import SearchLLM
import markdown
...
class Image2Text(QWidget):
    ...
    def __init__(self):
        ...
        self.beautify_button(self.ui.pushButtonSearch, SEARCH_URL)
    ...
    def job_search(self, words):
        self.search = SearchLLM(words)
        markdown_text = self.search.get()
        self.ui.textEdit.setText(markdown_text)
        
    def run_search_async(self,words):
        self.loadingLabel.setVisible(True)
        self.job_search(words)
        # threading.Thread(target=self.job_search, args=(words,)).start()
        self.loadingLabel.setVisible(False)
        
    @pyqtSlot()
    def on_pushButtonSearch_clicked(self):
        self.run_search_async(self.ui.textEdit.toPlainText())
